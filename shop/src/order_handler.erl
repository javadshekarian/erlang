...
handle_list(Req) ->
    case order_store:all() of
        {ok, List} ->
            Req2 = cowboy_req:reply(200, #{<<"content-type">> => <<"application/json">>}, jiffy:encode(List, [array_as_list]), Req),
            {ok, Req2, undefined};
        {error, _} ->
    ...
    case catch jiffy:decode(BodyBin, [return_maps]) of
        ...
        Map ->
            case order_store:create(Map) of
                {ok, Order} ->
                    Req3 = cowboy_req:reply(201, #{<<"content-type">> => <<"application/json">>}, jiffy:encode(Order), Req2),
                    {ok, Req3, undefined};
                {error, invalid_fk} ->
...
handle_get(Req, IdBin) ->
    ...
    case order_store:get(Id) of
        {ok, Order} ->
            Req2 = cowboy_req:reply(200, #{<<"content-type">>=><<"application/json">>}, jiffy:encode(Order), Req),
            {ok, Req2, undefined};
        {error, not_found} ->
    ...
    case catch jiffy:decode(BodyBin, [return_maps]) of
        ...
        Map ->
            case order_store:update(Id, Map) of
                {ok, Order} ->
                    Req3 = cowboy_req:reply(200, #{<<"content-type">> => <<"application/json">>}, jiffy:encode(Order), Req2),
                    {ok, Req3, undefined};
                {error, not_found} ->

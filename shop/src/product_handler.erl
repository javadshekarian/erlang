...
handle_list(Req) ->
    case product_store:all() of
        {ok, List} ->
            Body = jiffy:encode(List, [array_as_list]),
...
handle_get(Req, IdBin) ->
    ...
    case product_store:get(Id) of
        {ok, Product} ->
            Req2 = cowboy_req:reply(200, #{<<"content-type">>=><<"application/json">>}, jiffy:encode(Product), Req),
            {ok, Req2, undefined};
        {error, not_found} ->
    ...
    case catch jiffy:decode(BodyBin, [return_maps]) of
        ...
        Map ->
            case product_store:update(Id, Map) of
                {ok, Product} ->
                    Req3 = cowboy_req:reply(200, #{<<"content-type">> => <<"application/json">>}, jiffy:encode(Product), Req2),
                    {ok, Req3, undefined};
                {error, not_found} ->

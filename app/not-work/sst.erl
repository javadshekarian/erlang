...
handle_event(enter, _OldState, red, Data) -> 
    io:format("ENTER RED → state_timeout = 10s~nOldState: ~p~nData:~p~n",[_OldState,Data]),
    {keep_state_and_data, 10000};

handle_event(enter, _OldState, green, Data) -> 
    io:format("ENTER Green → state_timeout = 10s~nOldState: ~p~nData:~p~n",[_OldState,Data]),
    {keep_state_and_data, 10000};

handle_event(enter, _OldState, yellow, Data) -> 
    io:format("ENTER Yellow → state_timeout = 10s~nOldState: ~p~nData:~p~n",[_OldState,Data]),
    {keep_state_and_data, 10000};
